function RadarSignalGenerator_Batch()
    % 雷达信号数据集生成器 - 批次保存版本
    % 根据7架飞机的航线生成雷达信号数据集，分批次保存以优化内存使用
    
    fprintf('开始生成雷达信号数据集（批次保存模式）...\n');
    fprintf('============================================================\n');
    ...
    params.F = 3;                      % 噪声系数 (dB)
    params.num_samples_per_point = 1000; % 每个航点生成的样本数量
    params.pulse_width = 5e-6;         % 脉冲宽度 (s)
    params.fs = 100e6;                 % 采样率 (Hz)
    params.repeat_factor = 1;          % 信号重复因子
    
    % 批次保存参数
    params.batch_size = 500;          % 每批次样本数量（减小到2000，加载更快）
    params.output_dir = 'E:\BaiduNet_Download\dataGen\radar_500point';
    ...
    buffer_Y = [];
    buffer_info = {};
    sample_count = 0;      % 当前缓冲区样本数
    total_samples = 0;     % 全局样本计数
    batch_count = 0;       % 批次计数
    ...
    for i = 1:length(aircraft_names)
        ...
        total_waypoints = total_waypoints + size(waypoints.(ac_name), 1);
    end
    expected_total_samples = total_waypoints * params.num_samples_per_point;
    expected_batches = ceil(expected_total_samples / params.batch_size);
    
    fprintf('============================================================\n');
    fprintf('配置信息:\n');
    fprintf('  飞机数量: %d\n', length(aircraft_names));
    fprintf('  总航点数: %d\n', total_waypoints);
    fprintf('  每航点样本数: %d\n', params.num_samples_per_point);
    fprintf('  批次大小: %d 样本/批次\n', params.batch_size);
    ...
    for ac_idx = 1:length(aircraft_names)
        ...
        fprintf('\n[%d/%d] 正在处理: %s (%s)\n', ac_idx, length(aircraft_names), ac_name, ac_type);
        ...
        for wp_idx = 1:size(ac_waypoints, 1)
            ...
            % 显示航点信息
            if mod(wp_idx, 5) == 1 || wp_idx == size(ac_waypoints, 1)
                fprintf('  航点 %2d/%2d: 距离=%.2fkm, 位置=(%.2f°N, %.2f°E), SNR=%.2fdB\n', ...
                       wp_idx, size(ac_waypoints, 1), distance_km, wp(1), wp(2), SNR_dB);
            end
            ...
            if isempty(buffer_X)
                buffer_X = X_batch;
                buffer_Y = Y_batch;
                buffer_info = info_batch;
            else
                buffer_X = [buffer_X; X_batch];
                buffer_Y = [buffer_Y; Y_batch];
                buffer_info = [buffer_info; info_batch];
            end
            
            sample_count = sample_count + size(X_batch, 1);
            total_samples = total_samples + size(X_batch, 1);
            
            % 检查是否达到批次大小
            if sample_count >= params.batch_size
                % 保存当前批次
                batch_filename = sprintf('radar_batch_%04d.mat', batch_count);
                saveBatch(buffer_X, buffer_Y, buffer_info, batch_filename, ...
                         params.output_dir, batch_count, total_samples, params);
                
                fprintf('  ✓ 已保存批次 %d/%d (累计样本: %d/%d, %.1f%%)\n', ...
                       batch_count + 1, expected_batches, total_samples, ...
                       expected_total_samples, 100*total_samples/expected_total_samples);
...
function [X, Y, info] = generateAircraftSignals(ac, individual_idx, SNR_dB, num_samples, T, fs, distance_km, repeat_factor)
    ...
    for sample_idx = 1:num_samples
        ...
        if current_power > 0
            normalization_factor = 1 / sqrt(current_power);
...
function saveBatch(X_batch, Y_batch, info_batch, filename, output_dir, batch_idx, total_samples_so_far, params)
    % 保存单个批次到文件
    
    % 批次元数据
    batch_metadata = struct();
    batch_metadata.batch_index = batch_idx;
    batch_metadata.batch_size = size(X_batch, 1);
    batch_metadata.sample_range = [total_samples_so_far - size(X_batch, 1) + 1, total_samples_so_far];
    batch_metadata.signal_length = size(X_batch, 2);
    batch_metadata.timestamp = datestr(now, 'yyyy-mm-dd HH:MM:SS');
    batch_metadata.params = params;
    
    % 保存文件
    fullpath = fullfile(output_dir, filename);
    save(fullpath, 'X_batch', 'Y_batch', 'info_batch', 'batch_metadata', '-v7.3');
end
