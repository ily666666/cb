"""
RML2016 Dataset Reader
Dataset: RadioML 2016.10A - 6 modulation types with SNR variation
Signal format: IQ (In-phase and Quadrature) with 600 time samples
"""
import pickle
import numpy as np
import torch
...
class RML2016Dataset(Dataset):
    """
    RML2016 Dataset Loader (Optimized with caching)
    
    Structure: {(modulation_type, SNR): signal_array}
    Signal shape: (num_samples, 2, 600) where 2 is I/Q channels
    """
    ...
    def __init__(self, pkl_path, split='train', snr_range=None, seed=42, 
        ...
        """
        Args:
            pkl_path: Path to rml2016.pkl file
        ...
        self.modulation_types = ['16QAM', '64QAM', '8PSK', 'BPSK', 'GMSK', 'QPSK']
        self.mod_to_label = {mod: idx for idx, mod in enumerate(self.modulation_types)}
    ...
    def _load_data(self):
        ...
        if cache_key not in _RML2016_DATA_CACHE:
            load_start_time = time.time()
            print(f"正在加载 RML2016 数据集: {self.pkl_path}")
            ...
            print(f"   调制类型: {self.modulation_types}")
            ...
            for (mod_type, snr), signal_array in raw_data.items():
                ...
                if self.snr_range is not None:
                    if snr < self.snr_range[0] or snr > self.snr_range[1]:
                        continue
                label = self.mod_to_label[mod_type]
                filtered_data.append((signal_array, label))
                total_samples += len(signal_array)
            
            # 预分配数组（避免多次内存分配）
            first_shape = filtered_data[0][0].shape[1:]  # (2, 600)
            ...
            print(f"✅ 从缓存加载 RML2016 数据集 ({self.split})")
        
        # Get data from cache
        cached_data = _RML2016_DATA_CACHE[cache_key]
    ...
    def __getitem__(self, idx):
        """
        Returns:
            signal: Complex tensor of shape (600,) or (2, 600) depending on format
            label: Integer class label
        """
        signal = self.signals[idx]  # shape: (2, 600)
        label = self.labels[idx]
        
        # Convert to complex tensor: (2, 600) -> (600,) complex
...
def get_rml2016_dataloaders(pkl_path, batch_size=64, num_workers=0, snr_range=None, seed=42,
                            add_noise=False, noise_type='awgn', noise_snr_db=15, noise_factor=0.1):
    """
    Create train/val/test dataloaders for RML2016 dataset
    ...
    """
    train_dataset = RML2016Dataset(pkl_path, split='train', snr_range=snr_range, seed=seed,
                                   add_noise=add_noise, noise_type=noise_type, 
                                   noise_snr_db=noise_snr_db, noise_factor=noise_factor)
    val_dataset = RML2016Dataset(pkl_path, split='val', snr_range=snr_range, seed=seed,
                                 add_noise=add_noise, noise_type=noise_type, 
                                 noise_snr_db=noise_snr_db, noise_factor=noise_factor)
    test_dataset = RML2016Dataset(pkl_path, split='test', snr_range=snr_range, seed=seed,
                                  add_noise=add_noise, noise_type=noise_type, 
                                  noise_snr_db=noise_snr_db, noise_factor=noise_factor)
...
if __name__ == '__main__':
    ...
    pkl_path = r"E:\BaiduNet_Download\rml2016.pkl"
