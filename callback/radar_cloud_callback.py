...
@register_task
def radar_cloud_direct_infer_callback(task_id):
    """radar 端→云 直接推理回调"""
    return cloud_direct_infer_callback(task_id, config_prefix='radar_')


@register_task
def radar_cloud_infer_callback(task_id):
    """radar 端→边→云 协同推理回调（云侧部分）"""
    return cloud_infer_callback(task_id, config_prefix='radar_')
