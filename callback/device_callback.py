...
def _parse_pkl_data(data, dataset_type):
    ...
    def _infer_label(_key):
        ...
        if isinstance(_key, str):
            if label_map and _key in label_map:
                return label_map[_key]
            try:
                return int(_key)
            except ValueError:
                return None
    ...
    for key, signal_array in data.items():
        ...
        if arr.ndim < 2:
            print(f"[警告] signal_array 维度异常: {getattr(arr, 'shape', None)}, key={key}, 跳过")
            continue
        ...
        cur_sample_shape = tuple(arr.shape[1:])
        if sample_shape is None:
            sample_shape = cur_sample_shape
            x_dtype = arr.dtype
        else:
            if cur_sample_shape != sample_shape:
                raise ValueError(f"字典格式数据样本shape不一致: {cur_sample_shape} vs {sample_shape}, key={key}")
            if arr.dtype != x_dtype:
                x_dtype = np.result_type(x_dtype, arr.dtype)

        total_samples += n
        per_key_meta.append((key, int(label), n))

    if total_samples <= 0 or sample_shape is None:
        raise ValueError(f"无法从数据中提取样本, keys样例: {keys[:5]}")

    X_data = np.empty((total_samples, *sample_shape), dtype=x_dtype)
    y_data = np.empty((total_samples,), dtype=np.int64)

    offset = 0
    for key, label, n in per_key_meta:
        arr = data.get(key)
        if arr is None:
            raise ValueError(f"解析过程中数据缺失: key={key}")

        if not isinstance(arr, np.ndarray):
            arr = np.asarray(arr)

        if arr.dtype != x_dtype:
            arr = arr.astype(x_dtype, copy=False)

        X_data[offset:offset + n] = arr
        y_data[offset:offset + n] = label
        offset += n
...
def device_load_callback(task_id, **kwargs):
    ...
    if dataset_type == 'radar' and isinstance(X_data, np.ndarray) and X_data.ndim >= 3:
        ...
        if 'cloud_only' in task_id_lower:
            ...
            if length >= 1000:
                X_data_save = X_data[..., :500]
                print(f"[Radar处理] 边侧/协同任务：从1000截断到500")
            else:
                X_data_save = X_data
                print(f"[Radar处理] 边侧/协同任务：保持长度={length}")
        else:
