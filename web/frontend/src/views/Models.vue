  ...
  <div>
    ...
    <div class="card">
      ...
      <el-table :data="configList" size="small">
        ...
        <el-table-column prop="signal_length" label="信号长度" width="90" />
        <el-table-column prop="cloud_model" label="云侧模型">
          <template #default="{ row }">
            <el-tag type="primary" size="small">{{ row.cloud_model }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="edge_model" label="边侧模型">
          <template #default="{ row }">
            <el-tag type="success" size="small">{{ row.edge_model }}</el-tag>
          </template>
        </el-table-column>
      </el-table>
    </div>
    ...
    <div class="grid-2">
      ...
      <div class="card">
        ...
        <el-table :data="modelData.cloud_models" size="small" max-height="500">
          <el-table-column prop="name" label="模型文件" min-width="200" show-overflow-tooltip />
...
const configList = computed(() => {
  return Object.entries(modelConfig.value).map(([dataset, cfg]) => ({ dataset, ...cfg }))
})
