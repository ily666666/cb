...
class ResNet20Real(nn.Module):
    ...
    def forward(self, x, is_feat=False, preact=False):
        ...
        # 输入x的形状: (batch_size, 4096) 复数张量
        # 提取实部并reshape为(batch_size, 2, 64, 64)
        x_real = x.real  # (batch_size, 4096)
        x_imag = x.imag  # (batch_size, 4096)
        x_real = x_real.reshape(x_real.shape[0], 1, 64, 64)
        x_imag = x_imag.reshape(x_imag.shape[0], 1, 64, 64)
        x = torch.cat([x_real, x_imag], dim=1)  # (batch_size, 2, 64, 64)
