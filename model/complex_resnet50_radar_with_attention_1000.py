"""
Complex-valued ResNet50 with Attention for Radar Dataset (1000 input length)
Extracted from training file with TimeFreqAttention mechanism
Input: (batch_size, 1000) complex tensor
Reshape: (batch_size, 1, 40, 25) - maintains complex values
...
class CombinedModel(nn.Module):
    """
    Two-stage classifier with attention mechanism (1000 input length version)
    ...
    def __init__(self, num_classes=7):
        ...
        print("Initializing Complex ResNet50 with Attention for Radar Dataset (1000 input)...")
        ...
        print("Complex ResNet50 with Attention (1000 input) initialized successfully.")

    def forward(self, x, is_feat=False, preact=False, return_features=False):
        # Input: (batch_size, 1000) complex tensor
        # Reshape to: (batch_size, 1, 40, 25)
        out = x.reshape(x.shape[0], 1, 40, 25)
...
if __name__ == '__main__':
    ...
    # Create dummy input
    dummy_input = torch.randn(4, 1000, dtype=torch.complex64)
