"""
Complex-valued ResNet50 for RML2016 Dataset
Input: (batch_size, 600) complex tensor
Reshape: (batch_size, 1, 20, 30) - maintains complex values
...
class ComplexResNet50RML2016(nn.Module):
    """
    Complex-valued ResNet50 for RML2016 Dataset
    
    Input shape: (batch_size, 600) complex tensor
    Reshape to: (batch_size, 1, 20, 30)
    
    Architecture: ResNet50 with Bottleneck blocks (3-4-6-3)
    """
    
    def __init__(self, num_classes=6):
        super(ComplexResNet50RML2016, self).__init__()
        self.num_classes = num_classes
        
        print("Initializing Complex ResNet50 for RML2016 Dataset...")
    ...
    def forward(self, x, is_feat=False):
        """
        Args:
            x: Input tensor of shape (batch_size, 600) complex
        ...
        """
        # Input shape: (batch_size, 600) complex
        batch_size = x.shape[0]
        
        # Reshape to (batch_size, 1, 20, 30)
        x = x.reshape(batch_size, 1, 20, 30)
...
CombinedModel = ComplexResNet50RML2016


if __name__ == '__main__':
    # Test the model
    model = ComplexResNet50RML2016(num_classes=6)
    
    # Create dummy input
    dummy_input = torch.randn(4, 600, dtype=torch.complex64)
