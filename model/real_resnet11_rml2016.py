...
class ResNet11Real(nn.Module):
    def __init__(self, num_classes=6):
        super(ResNet11Real, self).__init__()
        ...
        self.intermediate_fc = nn.Linear(1024, 1000)  # 1024→1000（256*4=1024）
    ...
    def _make_layer(self, in_channels, out_channels, blocks, stride):
        ...
        if stride != 1 or in_channels != out_channels * Bottleneck.expansion:  # 注意这里使用Bottleneck.expansion=4
    ...
    def forward(self, x, is_feat=False, preact=False, return_reconstruction=False):
        ...
        # Reshape each to (batch_size, 1, 24, 25)
        x_real = x_real.reshape(batch_size, 1, 24, 25)
        x_imag = x_imag.reshape(batch_size, 1, 24, 25)
        
        # Concatenate to (batch_size, 2, 24, 25)
...
#             raise ValueError(
#                 f"输入张量元素数错误！期望 {expected_total_elems} (batch={batch_size}, 2×24×25), "
