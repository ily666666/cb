"""
Complex-valued ResNet50 with Attention for RML2016 Dataset
Extracted from training file with TimeFreqAttention mechanism
Input: (batch_size, 600) complex tensor
Reshape: (batch_size, 1, 20, 30) - maintains complex values (FIXED from 24x25)
...
class CombinedModel(nn.Module):
    ...
    NOTE: Fixed reshape from (1, 24, 25) to (1, 20, 30) for proper 600-element input
    """
    def __init__(self, num_classes=6):
        super(CombinedModel, self).__init__()
        self.is_complex = True
        print("Initializing Complex ResNet50 with Attention for RML2016 Dataset...")
    ...
    def forward(self, x, is_feat=False, preact=False, return_features=False):
        # Input: (batch_size, 600) complex tensor
        # Reshape to: (batch_size, 1, 20, 30) - FIXED from 24x25
        out = x.reshape(x.shape[0], 1, 20, 30)
